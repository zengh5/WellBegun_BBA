...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub,
                 ...
                 iteration=93, initial_query=30, tol=0.0001, sigma=0.0003,   # TtBA 0.0003 CGBA: 0.0002
                 verbose_control='Yes', well_begun=0):
        self.well_begun = well_begun     # where to initialize the starting point with our strategy
        ##################################
        self.model = model
        self.src_img = src_img

        sr = (self.src_img - ImageNet_mean) / ImageNet_std
        self.src_lbl = torch.argmax(self.model.forward(sr).data).item()
        ...
        if tar_img is not None:
            ta = (self.tar_img - ImageNet_mean) / ImageNet_std
            self.tar_lbl = torch.argmax(self.model.forward(ta).data).item()
    ...
    def is_adversarial(self, image):
        ...
        predict_label = torch.argmax(self.model.forward(image).data).item()
    ...
    def lf_only_combination(self, x_0, x_random):
        original_LF = LF_filter(x_0.cpu()).cuda()
        x_random_LF = LF_filter(x_random.cpu()).cuda()
        x_random_AC = x_random - x_random_LF
        ...
        while True:
            mid = (cln + adv) / 2.0
            num_calls += 1
            combined = torch.clamp(mid + x_random_AC, 0, 1)
            if self.is_adversarial(combined) == 1:
                adv = mid
            else:
                cln = mid
            if torch.norm(adv - cln).cpu().numpy() < self.tol or num_calls >= 100:
        ...
        # x_start_np = torch.clamp(adv + x_random_AC, 0, 1)[0].permute(1, 2, 0).cpu().numpy()
        # plt.imshow(x_start_np)
        # plt.show()
        return torch.clamp(adv + x_random_AC, 0, 1), num_calls
    ...
    def normal_vector_approximation_batch(self, x_boundary, q_max, random_noises):
        ...
        for j in range(num_batchs):
            if j == num_batchs - 1:
                ...
                current_batch = random_noises[
                                self.grad_estimator_batch_size * j:self.grad_estimator_batch_size * (j + 1)]
            ...
            noisy_boundary_tensor = (noisy_boundary_tensor - ImageNet_mean) / ImageNet_std
    ...
    def go_to_boundary_CGBA_H(self, x_s, eta_o, x_b):
        ...
        while True:
            m = (torch.sin(theta) * torch.cos(theta / (pow(2, num_calls))) / torch.sin(
                theta / (pow(2, num_calls))) - torch.cos(theta)).item()
    ...
    def go_to_boundary_CGBA(self, x_s, eta_o, x_b):
        ...
        while True:
            m = (torch.sin(theta.cpu()) * torch.cos(
                ...
                torch.tensor([(math.pi / 2)]) * (1 - 1 / pow(2, num_calls))) - torch.cos(theta.cpu())).item()
            zeta = (eta + m * v) / torch.norm(eta + m * v)
    ...
    def Attack(self):
        ...
        if self.tar_img is None:                         # untargeted mode
        ...
        if self.well_begun == 0:
            x_b, query_b = self.bin_search(self.src_img, x_random)               # baseline
        else:
            x_b, query_b = self.lf_only_combination(self.src_img, x_random)      # the proposed
        x_start_np = x_b[0].permute(1, 2, 0).cpu().numpy()
        plt.imshow(x_start_np)
        plt.show()
